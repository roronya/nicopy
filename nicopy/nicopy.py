...
def get_video_info(video_id, timeout=5):
    ...
    thumb_info =  {'video_id': soup.find('video_id').get_text(),
                   ...
                   'user_id': soup.find('user_id').get_text(),
                   'user_nickname': soup.find('user_nickname').get_text(),
                   'user_icon_url': soup.find('user_icon_url').get_text()}
    return thumb_info
...
def get_mylist_info(mylist_id, timeout=5):
    response = requests.get('http://www.nicovideo.jp/mylist/{0}?rss=2.0&lkang=ja-jp'.format(mylist_id), timeout=timeout)
...
def get_nicohistory_cookie(video_id, cookies, timeout=5):
    response = requests.get('http://www.nicovideo.jp/watch/' + video_id, cookies=cookies, timeout=timeout)
    return response.cookies
